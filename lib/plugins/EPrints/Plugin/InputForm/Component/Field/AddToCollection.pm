...
#	$page->appendChild( $session->make_element( 'input', id=>'collection_eprintid', type=>'hidden', value=>$eprint->get_id ) );
	$page->appendChild( $session->make_element( 'input', id=>'collection_eprintid', type=>'hidden', value=>$eprint->internal_uri ) );
...
sub _render_userdeposits_tab
{
	...
	my $list = $session->current_user->owned_eprints_list( $ds );
